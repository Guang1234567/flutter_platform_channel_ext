 ...
 */
public class ExtStandardMessageCodec implements MessageCodec<Object> {
    ...
    private static final byte FlutterParcelable = 14;
    ...
    protected void writeValue(ByteArrayOutputStream stream, Object value) {
        ...
        } else if (value instanceof FlutterParcelable) {
            stream.write(FlutterParcelable);
            ((FlutterParcelable) value).writeToParcel(new WriteParcel(this, stream));
        } else {
     ...
     */
    protected final <T, C> T readValue(ByteBuffer buffer, FlutterParcelable.Creator<C> creator) {
        if (!buffer.hasRemaining()) {
            throw new IllegalArgumentException("Message corrupted");
        }
        final byte type = buffer.get();
        return readValueOfType(type, buffer, creator);
    }

    protected final Object readValue(ByteBuffer buffer) {
        return this.readValue(buffer, null);
    }
     ...
     */
    protected <T, C> T readValueOfType(byte type, ByteBuffer buffer, FlutterParcelable.Creator<C> creator) {
        final Object result;
        switch (type) {
            case NULL:
                result = null;
            ...
            case LIST: {
                ...
                final List<C> list = new ArrayList<>(size);
                for (int i = 0; i < size; i++) {
                    list.add(this.<C, C>readValue(buffer, creator));
                }
                result = list;
                break;
            }
            case MAP: {
                final int size = readSize(buffer);
                final Map<C, C> map = new HashMap<>();
                for (int i = 0; i < size; i++) {
                    map.put(this.<C, C>readValue(buffer, creator), this.<C, C>readValue(buffer, creator));
                }
                result = map;
                break;
            }
            case FlutterParcelable: {
                if (creator == null) {
                    throw new IllegalArgumentException("Must not be null : FlutterParcelable.Creator");
                }
                result = creator.createFromParcel(new ReadParcel(this, buffer));
                break;
            }
            default:
                throw new IllegalArgumentException("Message corrupted");
